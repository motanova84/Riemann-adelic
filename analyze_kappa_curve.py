...
class KappaCurveAnalyzer:
    ...
    def analyze_convergence(self, window_size: int = 10) -> Dict:
        ...
        if len(relative_error) >= window_size:
            moving_avg = np.convolve(relative_error, np.ones(window_size) / window_size, mode="valid")
        else:
        ...
        return {
            ...
            "convergence_rate": (
                -np.polyfit(np.log(n_vals[1:]), np.log(relative_error[1:]), 1)[0] if len(n_vals) > 2 else np.nan
            ),
        }

    def visualize_kappa_curve(self, show_fit: bool = True, save_path: Optional[str] = None):
        """
        ...
        if show_fit and self.fit_params is not None:
            ...
            ax1.plot(
                n_vals, kappa_fit, "--", label=f"Fit: C={C:.3f}/(n log n) + {offset:.3f}", linewidth=2, color="red"
            )
        ...
        ax2.loglog(n_theory, kappa_theory, "--", label="Theory: 1/(n log n)", linewidth=2, color="green")
