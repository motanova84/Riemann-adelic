...
def plot_observational_horizon(x, horizon, psi, save_path=None):
    ...
    if len(horizon_x) > 0:
        plt.scatter(horizon_x, horizon_psi, c='r', s=100, marker='o', 
                   label='Horizon Points', zorder=5)
    plt.xlabel('Position x', fontsize=12)
    plt.ylabel('Ψ(x)', fontsize=12)
...
def plot_comparison_with_flat_space(N=100, n_eigen=10, save_path=None):
    ...
    plt.subplot(1, 2, 1)
    plt.plot(range(n_eigen), eigen_flat, 'ko-', markersize=8, linewidth=2, 
            label='Flat Space (Ψ=0)', alpha=0.7)
    plt.plot(range(n_eigen), eigen_curved, 'ro-', markersize=8, linewidth=2,
            label='Curved Space (Ψ≠0)')
    plt.xlabel('Eigenvalue Index n', fontsize=12)
...
def main():
    ...
    plot_observational_horizon(x, horizon, psi, 
                               save_path='output/curved_spacetime_horizon.png')
    
    # Comparison with flat space
    print("\n🔍 Comparing with flat spacetime...")
    plot_comparison_with_flat_space(N=N, n_eigen=n_eigenvalues,
                                   save_path='output/curved_vs_flat_comparison.png')
