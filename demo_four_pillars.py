...
from pillars.pilar1_spectral_inversion import compare_with_primes, spectral_inversion
...
from pillars.pilar3_uniqueness import characterize_xi_function, construct_pw_test_class, verify_uniqueness
...
def demo_pilar1():
    ...
    if peaks:
        ...
        for i, (pos, height) in enumerate(peaks[:10], 1):
            ...
            print(f"   Pico {i}: posición={pos:.4f}, altura={height:.6f}, " f"exp(pos)≈{prime_estimate:.2f}")
...
def demo_pilar4():
    ...
    H, eigenvalues, x_values = build_rh_operator(x_min=0.5, x_max=5.0, num_points=100, t=0.1)
    ...
    for i in range(min(5, len(comparison["implied_zeros"]))):
        ...
        if known is not None and diff is not None:
            print(f"      Cero {i+1}: implicado={impl:.6f}, conocido={known:.6f}, " f"diferencia={diff:.6f}")
