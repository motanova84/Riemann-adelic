...
from operators.hilbert_polya_fredholm import HilbertPolyaFredholmOperator, L2EvenHilbertSpace, generate_primes
...
def demo_arithmetic_potential():
    ...
    for p in primes[:5]:
        ...
        ax.axvline(x=-ln_p, color="blue", linestyle="--", alpha=0.5, linewidth=1)
        ax.text(ln_p, ax.get_ylim()[1] * 0.9, f"p={p}", ha="center", fontsize=8, rotation=90)
...
def demo_operator_spectrum():
    ...
    operator = HilbertPolyaFredholmOperator(u_max=8.0, n_points=201, n_primes=30, max_power=2)
    ...
    ax.axhline(y=1e-10, color="g", linestyle="--", linewidth=2, label="Machine precision (~10⁻¹⁰)")
    ax.set_xlabel("Index", fontsize=11)
    ...
    ax.axvline(x=np.mean(spacings), color="r", linestyle="--", linewidth=2, label=f"Mean = {np.mean(spacings):.3f}")
...
def demo_validation_summary():
    ...
    ax = axes[0, 1]
    ax.text(0.5, 0.8, f"QCAL ∞³ Integration", ha="center", fontsize=14, fontweight="bold", transform=ax.transAxes)
    ax.text(0.5, 0.65, f'f₀ = {result.parameters["f0_qcal"]} Hz', ha="center", fontsize=12, transform=ax.transAxes)
    ax.text(0.5, 0.5, f'C = {result.parameters["c_coherence"]}', ha="center", fontsize=12, transform=ax.transAxes)
    ax.text(
        0.5,
        0.35,
        f"Ψ = {result.psi:.3f}",
        ...
        color="green" if result.psi >= 0.8 else "orange",
    )
    ax.text(0.5, 0.2, f"Time: {result.computation_time_ms:.1f} ms", ha="center", fontsize=10, transform=ax.transAxes)
    ax.axis("off")

    # Plot 3: Mathematical Properties
    ax = axes[1, 0]
    properties = ["Self-Adjoint", "Real Spectrum", "Even Parity", "Fredholm Det.", "Hermitian"]
    status = ["✓", "✓", "✓", "✓", "✓"]
...
♾️³ Q.E.D. ADELANTE CONTINUA
    """
    ax.text(0.05, 0.95, summary_text, ha="left", va="top", fontsize=9, family="monospace", transform=ax.transAxes)
    ax.axis("off")
