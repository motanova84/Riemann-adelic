...
from utils.rh_resonators import COHERENCE_C, F0_FUNDAMENTAL, RHResonatorSystem
...
def plot_coherence_field(resonator: RHResonatorSystem, duration: float = 0.1):
    ...
    ax2.axvline(x=F0_FUNDAMENTAL, color="r", linestyle="--", label=f"f₀ = {F0_FUNDAMENTAL:.4f} Hz")
    ax2.axvline(
        x=resonator.dominant_frequency,
        color="orange",
        linestyle=":",
...
def plot_mode_structure(resonator: RHResonatorSystem, n_modes: int = 20):
    ...
    ax1.axhline(y=F0_FUNDAMENTAL, color="r", linestyle="--", label=f"f₀ = {F0_FUNDAMENTAL:.4f} Hz")
    ...
    ax3.plot(range(n_modes), cumulative, "b-", linewidth=2, marker="o", markersize=4, label="Cumulative")
