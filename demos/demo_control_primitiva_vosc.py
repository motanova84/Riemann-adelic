...
def visualizacion_2_supremo_acotado():
    ...
    axes[0, 0].axhline(sup_origen, color="r", linestyle="--", linewidth=1.5, label=f"sup = {sup_origen:.4f}")
    ...
    axes[0, 1].axhline(sup_intermedio, color="r", linestyle="--", linewidth=1.5, label=f"sup = {sup_intermedio:.4f}")
    ...
    axes[1, 0].axhline(sup_grande, color="r", linestyle="--", linewidth=1.5, label=f"sup = {sup_grande:.6f}")
    ...
    axes[1, 1].axhline(
        supremo_global, color="r", linestyle="--", linewidth=2, label=f"Supremo Global = {supremo_global:.4f}"
    )
    ...
    for bar, val in zip(bars, supremos):
        ...
        axes[1, 1].text(bar.get_x() + bar.get_width() / 2.0, height, f"{val:.4f}", ha="center", va="bottom", fontsize=9)
...
def visualizacion_3_montgomery_vaughan():
    ...
    axes[0].plot(T_values, integrales_empiricas, "bo-", linewidth=2, markersize=6, label="∫₀ᵀ |W(x)|² dx (empírico)")
    axes[0].plot(T_values, valores_teoricos, "r--", linewidth=2, label="T log log P (teórico MV)")
    axes[0].set_xlabel("T", fontsize=12)
    axes[0].set_ylabel("Integral", fontsize=12)
    axes[0].set_title("Teorema de Montgomery-Vaughan: ∫|W|² ~ T log log P", fontsize=14, fontweight="bold")
    axes[0].legend(fontsize=11)
    axes[0].grid(True, alpha=0.3)

    # Panel 2: Error relativo
    error_relativo = np.abs(integrales_empiricas - valores_teoricos) / valores_teoricos
    axes[1].plot(T_values, error_relativo * 100, "g^-", linewidth=2, markersize=6, label="Error relativo")
    axes[1].axhline(50, color="r", linestyle="--", linewidth=1.5, alpha=0.5, label="50% threshold")
