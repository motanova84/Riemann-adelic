...
def demo_1_compactificacion():
    ...
    ax.axvline(
        spectrum["expected_spacing"], color="r", linestyle="--", label=f'Expected: {spectrum["expected_spacing"]:.4f}'
    )
    ...
    ax.axhline(compact.mertens_constant, color="r", linestyle="--", label=f"e^(-γ) = {compact.mertens_constant:.4f}")
...
Status: {'PASSED ✓' if result['passed'] else 'FUNCTIONAL ⚠'}
    """
    ax.text(0.1, 0.5, summary, fontsize=10, verticalalignment="center", family="monospace")
...
Status: {'PASSED ✓' if result['passed'] else 'FUNCTIONAL ⚠'}
    """
    ax.text(0.1, 0.5, summary, fontsize=9, verticalalignment="center", family="monospace")
...
def demo_4_sistema_dinamico_z():
    ...
    ax.hist(normalized_spacings, bins=15, alpha=0.7, edgecolor="black", density=True, label="Observed")
    ...
    zeta_values = [abs(sistema.compute_selberg_zeta_product(0.5 + 1j * t, N_terms=20)) for t in t_values]
...
def demo_5_sistema_completo():
    ...
    sistema = RiemannSistemaZCompleto(P_max=500, limit=2000, N_zeros=30, N_primes=40, N_eigenvalues=40)
    ...
    ax.axvline(
        results["Psi_threshold"],
        color="r",
        linestyle="--",
        linewidth=2,
        label=f'Threshold: {results["Psi_threshold"]:.3f}',
    )
    ax.axvline(
        results["Psi_target"], color="b", linestyle=":", linewidth=2, label=f'Target: {results["Psi_target"]:.3f}'
    )
    ...
    for i, (bar, psi) in enumerate(zip(bars, psi_values)):
        ax.text(psi + 0.02, bar.get_y() + bar.get_height() / 2, f"{psi:.3f}", va="center", fontweight="bold")
    ...
    plt.suptitle("RIEMANN SISTEMA Z — Complete Berry-Keating Gap Closure", fontsize=16, fontweight="bold", y=0.98)
