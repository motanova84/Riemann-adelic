...
def visualizacion_4_dinamico_z():
    ...
    ax.axhline(
        LYAPUNOV_Z, color="g", linestyle="--", linewidth=2, label=f"$\\lambda = \\log(\\phi) = {LYAPUNOV_Z:.6f}$"
    )
    ...
    counts, bins, patches = ax.hist(
        normalized_spacings, bins=15, density=True, color="blue", alpha=0.6, edgecolor="black", label="Datos"
    )
...
def visualizacion_5_sistema_completo():
    ...
    ax.fill_between(
        r * np.cos(theta_fill), 0, r * np.sin(theta_fill), color="green" if Psi_global >= 0.95 else "orange", alpha=0.6
    )
    ...
    ax.text(
        0,
        -0.5,
        "VALIDATED" if all_valid else "IN PROGRESS",
        ha="center",
        fontsize=14,
        fontweight="bold",
...
def dashboard_unificado():
    ...
    ax1a.plot(n_conf, eigs_conf, "o-", color=pillar_colors["compactificacion"], markersize=3, linewidth=1)
    ...
    if len(eigs_conf) > 1:
        ...
        ax1b.hist(gaps_conf, bins=20, color=pillar_colors["compactificacion"], alpha=0.7, edgecolor="black")
    ...
    ax2b.plot(x_err_vals, psi_errs, "-", color=pillar_colors["filtro"], linewidth=2, label="$\\psi(x) - x$")
    ...
    factors = [filtro.compute_mobius_cancellation(N=N)["cancellation_factor"] for N in N_vals_mob]
    ax2c.semilogx(N_vals_mob, factors, "o-", color=pillar_colors["filtro"], linewidth=2, markersize=6)
    ...
    ax4b.hist(
        ...
        bins=25,
        density=True,
        color=pillar_colors["dinamico"],
        alpha=0.7,
        edgecolor="black",
        label="Datos (100 ceros)",
    )
