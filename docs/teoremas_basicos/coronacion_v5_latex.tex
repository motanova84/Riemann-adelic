...
\begin{definition}[Dominio denso]\label{def:dominio-h-latex}
Sea
\[
...
\begin{theorem}[Autoadjunción esencial]\label{thm:autoadjuncion-latex}
El operador
\[
...
\begin{theorem}[Fórmula de traza exacta]\label{thm:traza-exacta-latex}
Para toda $f \in C_c^\infty(\mathbb{R})$ con transformada de Fourier $\widehat{f} \in C_c^\infty(\mathbb{R})$ se tiene
\[
  \mathrm{Tr}(f(\widehat{A})) = \sum_{\gamma_n} f(\gamma_n) + \frac{1}{2\pi} \int_{\mathbb{R}} f(t)\left[\log \pi - \frac{\Gamma'}{\Gamma}\Big(\frac{1}{4} + \frac{it}{2}\Big)\right] dt.
\]
\end{theorem}
...
\begin{enumerate}
  \item Consideramos $f_t(x) = e^{-itx}$ y calculamos $\mathrm{Tr}(e^{-it \widehat{A}})$ usando la representación de Weyl.
  ...
  \]
  \item Se verifica que $\mathrm{Tr}(e^{-it \widehat{A}})$ reproduce exactamente los términos geométricos.
  \item La densidad de las funciones de prueba en $C_c^\infty$ asegura la igualdad espectral.
\end{enumerate}
\end{proof}

\begin{corollary}[Correspondencia espectral]\label{cor:correspondencia-espectral-latex}
...
\[
  \zeta_{Z_\sigma, Z}(z,s) = \mathrm{Tr}\left[(Z_\sigma^2 + (s-\tfrac{1}{2})^2)^{-z} - (Z^2 + (s-\tfrac{1}{2})^2)^{-z}\right].
\]

\begin{theorem}[Convergencia meromorfa]\label{thm:determinante-latex}
...
\begin{lemma}[Volumen de fase]\label{lem:volumen-fase-latex}
Para $T$ grande,
\[
  \mathrm{Vol}\{(x,p) : |\sigma(x,p)| \le T\} = \frac{2T}{\pi} L - \frac{T}{\pi} + O(1).
\]
\end{lemma}
...
\begin{theorem}[Ley de Weyl]\label{thm:weyl-latex}
...
\begin{theorem}[No degeneración espectral]\label{thm:simple-latex}
