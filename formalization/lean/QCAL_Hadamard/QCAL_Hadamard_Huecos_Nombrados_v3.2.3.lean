...
/-!
  Mapa v3.2.3 — 27 ago 2026 (coherencia)

  Enunciado: enteras de orden ≤ 1, mismos ceros con multiplicidad,
  f(1-s)=f(s), g(1-s)=g(s), f(1/2)=g(1/2)≠0 ⇒ f=g.
  Hadamard, no densidad de ceros (falso: γ_n aislados).

  Cerrado como argumento
  ----------------------
  B=0 por F(s)=exp(B(1-2s))≡1 ⇒ F'(0)=0.
  C=e^A=1 por normalización en 1/2.
  ξ es OrderAtMostOne, no Paley–Wiener.

  Tres huecos (archivos)
  ----------------------
  GAP1  GAP1_log_holomorphic_of_entire_never_zero.lean
        log continuo Mathlib; falta biholomorfismo local de exp
        y pegado 2πiℤ constante.
  GAP2  GAP2_affine_log_of_order_one.lean
        Borel + Cauchy n=2 + φ''=0. DiffContOnCl cerrado.
        Glue: K de Borel, tendsto R^{-3/2}, fderiv 0 ⇒ afín.
  GAP3  GAP3_quotient_entire_never_zero.lean
        h = f^{(n)}/g^{(n)}, n=ord_z g. Sin extract_zeros_poles.
        Orden ≠ ⊤ y h≠0 nombrados. Glue: Riemann extraíble,
        anillo en el agujero, f=hg en el cero.

  No se afirma
  ------------
  RH. D ≡ Ξ. lake-checked. Paley–Wiener para ξ.

  Siguiente acto soberano: `lake build` en el nodo de José.
-/

end

/-! ## Canon v3.2.3 — nombres de José -/

/-- GAP 1. Log continuo Mathlib → holomorfo. -/
theorem log_holomorphic_of_entire_never_zero {h : ℂ → ℂ}
  ...
  exact ⟨φ, differentiable_of_exp_comp hh hφc hexp, hexp⟩

/-- GAP 2. Borel + Cauchy + Liouville ⇒ φ afín. -/
theorem affine_log_of_order_one {φ : ℂ → ℂ}
    (hφ : Differentiable ℂ φ)
    (hRe : ∀ ε > 0, ∃ C : ℝ, 0 < C ∧ ∀ z, (φ z).re ≤ C * (1 + ‖z‖ ^ (1 + ε))) :
    ∃ A B : ℂ, ∀ s, φ s = A + B * s :=
  entire_of_realPart_order_le_one hφ hRe

/-- GAP 3. Cociente con infinitos ceros. -/
theorem quotient_entire_never_zero {f g : ℂ → ℂ}
    (hf : Differentiable ℂ f) (hg : Differentiable ℂ g)
    (hzeros : SameZeros f g)
    (hg0 : ¬ ∀ z, g z = 0) :
    ∃ h : ℂ → ℂ, Differentiable ℂ h ∧ (∀ z, h z ≠ 0) ∧ ∀ z, f z = h z * g z :=
  exists_entire_quotient hf hg hzeros hg0
