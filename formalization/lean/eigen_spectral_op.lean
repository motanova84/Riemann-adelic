...
lemma eigenfunctions_orthonormal : Orthonormal_H H_eigenfunctions := by
  have h := H_spectrum_real.choose_spec
  exact h.1

/-- Each eigenfunction satisfies the eigenvalue equation -/
lemma eigenvalue_equation (n : ℕ) : 
    H_Psi (H_eigenfunctions n) = Complex.ofReal (H_eigenvalues n) • (H_eigenfunctions n) := by
  have h := H_spectrum_real.choose_spec
  exact h.2 n
...
theorem zeros_on_critical_line : ∀ s : ℂ, 
  ...
  -- Therefore Re(s) = 1/2 + Re(γ_n) = 1/2 + γ_n (but γ_n contributes only to Im)
