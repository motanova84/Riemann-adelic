...
from validate_gauge_conjugation import V_potential, free_operator, gauge_operator, hamiltonian_H_Psi, phase_function
...
def test_real_spectrum_from_self_adjointness():
    ...
    return {"eigenvalues": [float(e.real) for e in eigenvalues], "max_imag": float(max_imag), "passed": bool(passed)}
...
def test_spectral_correspondence_to_riemann_zeros():
    ...
    # From Odlyzko tables
    riemann_zeros_gamma = [14.134725, 21.022040, 25.010858, 30.424876, 32.935062]
...
def run_integration_tests():
    ...
    # Overall summary
    all_passed = all(results[key].get("passed", False) for key in results.keys())
    ...
    print(
        f"Test 3 - Spectral Correspondence: {'✓ PASS' if results['test3_spectral_correspondence']['passed'] else '✗ FAIL'}"
    )
