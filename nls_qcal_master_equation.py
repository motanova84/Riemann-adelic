...
class QCALParameters:
    ...
    def __repr__(self) -> str:
        return (
            f"QCALParameters(f₀={self.f0} Hz, γ₀={self.gamma_0}, " f"C={self.C}, coherence≥{self.coherence_threshold})"
        )
...
def compute_alpha(psi: np.ndarray, velocity_field: Optional[np.ndarray] = None, gamma_0: float = GAMMA_0) -> np.ndarray:
    """
    ...
    if velocity_field is not None:
        ...
        if velocity_field.ndim == 1:
            ...
            # 2D case
            div_v = np.gradient(velocity_field[0], axis=0) + np.gradient(velocity_field[1], axis=1)
        elif velocity_field.ndim == 3:
            # 3D case
            div_v = (
                np.gradient(velocity_field[0], axis=0)
...
def compute_energy(psi: np.ndarray, dx: float = 1.0, f0: float = F0) -> float:
    """
...
def compute_entropy(psi: np.ndarray, dx: float = 1.0) -> float:
    """
...
class NLS_QCAL_Solver:
    ...
    def __init__(self, params: Optional[QCALParameters] = None, N: int = 128, L: float = 10.0):
        """
        ...
        # History tracking
        self.history = {"time": [], "energy": [], "entropy": [], "coherence": [], "mass": []}
    ...
    def rhs(self, t: float, psi_real_imag: np.ndarray, velocity_field: Optional[np.ndarray] = None) -> np.ndarray:
        """
    ...
    def evolve(
        ...
        sol = solve_ivp(rhs_wrapper, t_span, y0, method=method, t_eval=t_eval, rtol=1e-6, atol=1e-8)
...
def create_gaussian_ic(x: np.ndarray, amplitude: float = 1.0, width: float = 1.0, center: float = 0.0) -> np.ndarray:
    """
...
def create_coherent_ic(x: np.ndarray, coherence: float = 0.95, phase: Optional[np.ndarray] = None) -> np.ndarray:
    """
...
def verify_global_existence(psi_0: np.ndarray, params: Optional[QCALParameters] = None) -> Dict:
    """
