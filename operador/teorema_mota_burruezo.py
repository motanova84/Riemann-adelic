...
class MotaBurruezoOperator:
    ...
    def __init__(self, config: Optional[OperatorHConfig] = None):
        ...
        self.log_x_grid = np.linspace(mp.log(self.config.x_min), mp.log(self.config.x_max), self.config.grid_size)
    ...
    def verify_critical_line_property(
        self, num_eigenvalues: int = 100, tolerance: float = 1e-2
...
def demonstrate_theorem():
    ...
    print("=" * 80)
    all_on_line, max_dev, eigenvalues = H_op.verify_critical_line_property(num_eigenvalues=50)
