...
class BiologicalPerturbation:
    ...
    def __init__(
        ...
        self.interpolator = interp1d(
            signal_time, self.signal_normalized, kind="cubic", bounds_error=False, fill_value=0.0
        )
    ...
    def to_rank1_perturbation(self, operator_grid: np.ndarray, spatial_width: Optional[float] = None) -> np.ndarray:
        """
        ...
        perturbation_matrix = self.perturbation_strength * np.outer(signal_at_grid, spatial_profile)

        return perturbation_matrix

    def to_local_potential_perturbation(self, operator_grid: np.ndarray, locality_scale: float = 1.0) -> np.ndarray:
        """
        ...
        smoothed_signal = gaussian_filter1d(signal_at_grid, sigma=locality_scale)
...
class PerturbedXiOperator:
    ...
    def _generate_perturbation_matrix(self) -> np.ndarray:
        ...
        if self.perturbation_type == "diagonal":
            ...
            diag_pert = self.perturbation.to_local_potential_perturbation(self.operator_grid)
            return np.diag(diag_pert)
    ...
    def _ensure_hermiticity(self):
        ...
        # Symmetrize: H = (H + H†) / 2
        self.perturbed_hamiltonian = (self.perturbed_hamiltonian + self.perturbed_hamiltonian.conj().T) / 2
    ...
    def compute_spectrum(self) -> Dict:
        ...
        return {"eigenvalues": eigenvalues, "eigenvectors": eigenvectors, "n_eigenvalues": len(eigenvalues)}
    ...
    def compute_spectral_shift(self, base_eigenvalues: np.ndarray) -> Tuple[float, float]:
        """
...
def create_hrv_perturbation(hrv_signal: Dict, perturbation_strength: float = 0.01) -> BiologicalPerturbation:
    """
    ...
    return BiologicalPerturbation(
        signal_time=time, signal_values=signal, perturbation_strength=perturbation_strength, signal_type="hrv"
    )


def create_microtubule_perturbation(mt_signal: Dict, perturbation_strength: float = 0.01) -> BiologicalPerturbation:
    """
...
if __name__ == "__main__":
    ...
    # Create perturbation
    pert = BiologicalPerturbation(signal_time=t, signal_values=hrv_signal, perturbation_strength=0.01)
