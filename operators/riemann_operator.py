...
def oscillatory_weight(x: np.ndarray, gamma_n: np.ndarray, alpha: float = 0.5, sigma: float = 10.0) -> np.ndarray:
    """
...
def kinetic_operator_matrix(n_basis: int, x_range: Tuple[float, float] = (-5.0, 5.0)) -> np.ndarray:
    ...
    deriv_matrix = diags([deriv_lower, deriv_upper], [-1, 1], shape=(n_basis, n_basis)).toarray()
...
def construct_H_psi_direct(n_zeros: int = 50, data_dir: Optional[str] = None) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def construct_H_psi(
    ...
    V = potential_operator_matrix(n_basis, gamma_n, x_range=x_range, alpha=alpha, sigma=sigma)
...
def validate_spectrum(eigenvalues: np.ndarray, gamma_n: np.ndarray, n_compare: Optional[int] = None) -> dict:
    """
...
def main():
    ...
    H_psi, gamma_n = construct_H_psi(n_basis=n_basis, n_zeros=n_zeros, x_range=x_range, alpha=alpha, sigma=sigma)
