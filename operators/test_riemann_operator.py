...
class TestConstructiveTheorem:
    ...
    def test_theorem_statement(self):
        ...
        assert results["max_abs_error"] < 1.5e-12, "Constructive theorem: max error must be < 1.5e-12"
        assert results["n_compared"] == 50, "Constructive theorem: must validate 50 zeros"

        # All eigenvalues must be real
        assert np.all(np.isreal(eigenvalues)), "Constructive theorem: eigenvalues must be real"

        # Operator must be Hermitian
        assert np.max(np.abs(H_psi - H_psi.T)) < 1e-14, "Constructive theorem: operator must be Hermitian"
