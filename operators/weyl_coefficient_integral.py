...
class WeylCoefficientIntegral:
    ...
    def compute_I_lambda_asymptotic(self, lambda_val: float) -> Tuple[float, float, float]:
        ...
        L = np.log(1 + y_plus)
        L_asymptotic = (0.5 / np.sqrt(self.alpha)) * np.log(lambda_val) + \
    ...
    def compute_I_lambda_numerical(self, lambda_val: float, 
...
def generate_weyl_coefficient_certificate(alpha: float = ALPHA_CORRECTED,
                                          lambda_test: float = 1000.0) -> Dict[str, Any]:
    """
