...
def validate_abc_complete(max_height: int = 10**12, epsilon: float = 1e-6,
    ...
    exceptional = find_exceptional_triples(
        practical_limit, 
        epsilon=epsilon,
        min_quality=1.0
    )
        ...
        'special_cases_summary': [
            {
                ...
                'triple': sc['triple'],
                'quality': round(math.log(sc['triple'][2]) / 
                               math.log(sc['verification']['rad_abc']), 6),
    ...
    if verbose:
        ...
        for t in top_triples[:15]:
            ...
            print(f"{t['a']:8d} {t['b']:8d} {t['c']:8d} {t['rad_abc']:10d} "
...
def main():
    ...
    parser.add_argument('--max-height', type=int, default=50000,
                       help='Maximum height to search (default: 50000)')
    ...
    parser.add_argument('--show-theory', action='store_true',
                       help='Display theoretical framework summary')
