...
class CircleMethodValidator:
    ...
    def test_1_major_arc_threshold(self):
        ...
        for N in test_values:
            ...
            print(f"N = {N:6d}: threshold = {threshold:.6f}, "
                  f"N^(1/4) = {expected_order:.2f}")
        ...
        self.results['test_1'] = {
            'name': 'Major Arc Threshold Scaling',
            'status': 'PASSED',
            'details': results
        }
        print("✓ Test 1 PASSED\n")
    ...
    def test_2_singular_series_positivity(self):
        ...
        self.results['test_2'] = {
            'name': 'Singular Series Positivity',
            'status': 'PASSED',
            'details': results
        }
        print("✓ Test 2 PASSED\n")
    ...
    def test_3_arc_partition_coverage(self):
        ...
        num_major_arcs = sum(sum(1 for a in range(q) if np.gcd(a, q) == 1) 
    ...
    def test_4_exponential_sum_decay(self):
        ...
        for alpha in test_alphas:
            ...
            print(f"α = {alpha_mod:.6f}: |S| = {magnitude:.2f}, "
                  f"|S|/N = {relative_size:.6f}")
            
            assert relative_size < 0.5, f"Insufficient cancellation at α={alpha_mod}"
            
            results.append({
                'alpha': float(alpha_mod),
                'magnitude': float(magnitude),
                'relative_size': float(relative_size)
            })
        
        self.results['test_4'] = {
            'name': 'Exponential Sum Decay',
            'status': 'PASSED',
            'details': results
        }
        print("✓ Test 4 PASSED\n")
    ...
    def test_5_goldbach_numerically_small_n(self):
        ...
        self.results['test_5'] = {
            'name': 'Goldbach Numerical Verification',
            'status': 'PASSED',
            'details': results
        }
        print("✓ Test 5 PASSED\n")
    ...
    def test_6_qcal_constants_consistency(self):
        ...
        self.results['test_6'] = {
            ...
            'details': {
                ...
                'threshold_test': float(threshold)
            }
        }
        print("✓ Test 6 PASSED\n")
        return True
