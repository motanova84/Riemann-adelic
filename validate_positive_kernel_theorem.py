...
from positive_kernel_rh_theorem import HilbertSchmidtOperator, PositiveDefiniteKernel, RiemannZetaSpectralConnection
...
def validate_kernel_positivity():
    ...
    for kernel_type in kernel_types:
        ...
        symmetry_ok = all(kernel.verify_symmetry(x, y) for x, y in test_pairs)
    ...
    all_passed = all(r["symmetric"] and r["positive_definite"] for r in results.values())
...
def validate_spectrum_nonnegative():
    ...
    for n_basis in basis_sizes:
        ...
        print(
            f"    All non-negative: {all_nonnegative} ✓"
            if all_nonnegative
            else f"    All non-negative: {all_nonnegative} ✗"
        )
...
def generate_validation_report(all_results: dict):
    ...
    report = {
        ...
        "overall_status": "PASSED" if all(r.get("passed", False) for r in all_results.values()) else "FAILED",
    }
