...
from xi_operator_simbiosis import F0, XiOperatorSimbiosis, run_xi_spectral_verification
...
def validate_xi_operator_complete(n_dim: int = 4096, t_max: float = 50.0) -> dict:
    ...
    verifier = Atlas3SpectralVerifier(node_id="xi_operator_simbiosis", beacon_dir=repo_root / "data" / "beacons")
    ...
    try:
        ...
        coherence_psi = verifier.calculate_coherence_psi(cla_result, gue_result, rigidity_result)
    ...
    certificate = {
        ...
        "parameters": {"n_dim": n_dim, "t_max": t_max, "frequency_base": F0},
        "spectrum_summary": {
            "total_eigenvalues": len(eigenvalues),
            "critical_line_count": len(spectrum["critical_eigenvalues"]),
            "t_range": (
                [float(spectrum["t_zeros"].min()), float(spectrum["t_zeros"].max())]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Validate Xi Operator Simbiosis with QCAL certification")
    parser.add_argument("--n-dim", type=int, default=4096, help="Hilbert space dimension (default: 4096)")
    parser.add_argument("--t-max", type=float, default=50.0, help="Maximum t range (default: 50.0)")

    args = parser.parse_args()

    # Run validation
    certificate = validate_xi_operator_complete(n_dim=args.n_dim, t_max=args.t_max)
